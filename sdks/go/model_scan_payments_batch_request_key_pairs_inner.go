/*
Sipher — Privacy-as-a-Skill API

REST API wrapping SIP Protocol's privacy SDK for Solana agents. Stealth addresses, Pedersen commitments, shielded transfers, and viewing key compliance.

API version: 0.1.0
Contact: hello@sip-protocol.org
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sipher

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ScanPaymentsBatchRequestKeyPairsInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ScanPaymentsBatchRequestKeyPairsInner{}

// ScanPaymentsBatchRequestKeyPairsInner struct for ScanPaymentsBatchRequestKeyPairsInner
type ScanPaymentsBatchRequestKeyPairsInner struct {
	// 0x-prefixed 32-byte hex string
	ViewingPrivateKey string `json:"viewingPrivateKey" validate:"regexp=^0x[0-9a-fA-F]{64}$"`
	// 0x-prefixed 32-byte hex string
	SpendingPublicKey string `json:"spendingPublicKey" validate:"regexp=^0x[0-9a-fA-F]{64}$"`
	Label *string `json:"label,omitempty"`
}

type _ScanPaymentsBatchRequestKeyPairsInner ScanPaymentsBatchRequestKeyPairsInner

// NewScanPaymentsBatchRequestKeyPairsInner instantiates a new ScanPaymentsBatchRequestKeyPairsInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewScanPaymentsBatchRequestKeyPairsInner(viewingPrivateKey string, spendingPublicKey string) *ScanPaymentsBatchRequestKeyPairsInner {
	this := ScanPaymentsBatchRequestKeyPairsInner{}
	this.ViewingPrivateKey = viewingPrivateKey
	this.SpendingPublicKey = spendingPublicKey
	return &this
}

// NewScanPaymentsBatchRequestKeyPairsInnerWithDefaults instantiates a new ScanPaymentsBatchRequestKeyPairsInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewScanPaymentsBatchRequestKeyPairsInnerWithDefaults() *ScanPaymentsBatchRequestKeyPairsInner {
	this := ScanPaymentsBatchRequestKeyPairsInner{}
	return &this
}

// GetViewingPrivateKey returns the ViewingPrivateKey field value
func (o *ScanPaymentsBatchRequestKeyPairsInner) GetViewingPrivateKey() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ViewingPrivateKey
}

// GetViewingPrivateKeyOk returns a tuple with the ViewingPrivateKey field value
// and a boolean to check if the value has been set.
func (o *ScanPaymentsBatchRequestKeyPairsInner) GetViewingPrivateKeyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ViewingPrivateKey, true
}

// SetViewingPrivateKey sets field value
func (o *ScanPaymentsBatchRequestKeyPairsInner) SetViewingPrivateKey(v string) {
	o.ViewingPrivateKey = v
}

// GetSpendingPublicKey returns the SpendingPublicKey field value
func (o *ScanPaymentsBatchRequestKeyPairsInner) GetSpendingPublicKey() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SpendingPublicKey
}

// GetSpendingPublicKeyOk returns a tuple with the SpendingPublicKey field value
// and a boolean to check if the value has been set.
func (o *ScanPaymentsBatchRequestKeyPairsInner) GetSpendingPublicKeyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SpendingPublicKey, true
}

// SetSpendingPublicKey sets field value
func (o *ScanPaymentsBatchRequestKeyPairsInner) SetSpendingPublicKey(v string) {
	o.SpendingPublicKey = v
}

// GetLabel returns the Label field value if set, zero value otherwise.
func (o *ScanPaymentsBatchRequestKeyPairsInner) GetLabel() string {
	if o == nil || IsNil(o.Label) {
		var ret string
		return ret
	}
	return *o.Label
}

// GetLabelOk returns a tuple with the Label field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ScanPaymentsBatchRequestKeyPairsInner) GetLabelOk() (*string, bool) {
	if o == nil || IsNil(o.Label) {
		return nil, false
	}
	return o.Label, true
}

// HasLabel returns a boolean if a field has been set.
func (o *ScanPaymentsBatchRequestKeyPairsInner) HasLabel() bool {
	if o != nil && !IsNil(o.Label) {
		return true
	}

	return false
}

// SetLabel gets a reference to the given string and assigns it to the Label field.
func (o *ScanPaymentsBatchRequestKeyPairsInner) SetLabel(v string) {
	o.Label = &v
}

func (o ScanPaymentsBatchRequestKeyPairsInner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ScanPaymentsBatchRequestKeyPairsInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["viewingPrivateKey"] = o.ViewingPrivateKey
	toSerialize["spendingPublicKey"] = o.SpendingPublicKey
	if !IsNil(o.Label) {
		toSerialize["label"] = o.Label
	}
	return toSerialize, nil
}

func (o *ScanPaymentsBatchRequestKeyPairsInner) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"viewingPrivateKey",
		"spendingPublicKey",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varScanPaymentsBatchRequestKeyPairsInner := _ScanPaymentsBatchRequestKeyPairsInner{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varScanPaymentsBatchRequestKeyPairsInner)

	if err != nil {
		return err
	}

	*o = ScanPaymentsBatchRequestKeyPairsInner(varScanPaymentsBatchRequestKeyPairsInner)

	return err
}

type NullableScanPaymentsBatchRequestKeyPairsInner struct {
	value *ScanPaymentsBatchRequestKeyPairsInner
	isSet bool
}

func (v NullableScanPaymentsBatchRequestKeyPairsInner) Get() *ScanPaymentsBatchRequestKeyPairsInner {
	return v.value
}

func (v *NullableScanPaymentsBatchRequestKeyPairsInner) Set(val *ScanPaymentsBatchRequestKeyPairsInner) {
	v.value = val
	v.isSet = true
}

func (v NullableScanPaymentsBatchRequestKeyPairsInner) IsSet() bool {
	return v.isSet
}

func (v *NullableScanPaymentsBatchRequestKeyPairsInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableScanPaymentsBatchRequestKeyPairsInner(val *ScanPaymentsBatchRequestKeyPairsInner) *NullableScanPaymentsBatchRequestKeyPairsInner {
	return &NullableScanPaymentsBatchRequestKeyPairsInner{value: val, isSet: true}
}

func (v NullableScanPaymentsBatchRequestKeyPairsInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableScanPaymentsBatchRequestKeyPairsInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


