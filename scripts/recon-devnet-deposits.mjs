...
// DepositRecord layout (sipher_vault Anchor program):
// 8 (discriminator) + 32 (depositor) + 32 (token_mint) + 8 (amount, u64)
// + 8 (last_deposit_at, i64) + 8 (deposit_count, u64) + 1 (bump) = 97 bytes
const DEPOSIT_RECORD_SIZE = 97
...
for (const { pubkey, account } of accounts) {
  ...
  const depositCount = Number(data.readBigInt64LE(offset)); offset += 8
  const bump = data.readUInt8(offset)

  const ageSeconds = now - lastDepositAt
  const refundable = ageSeconds >= TIMEOUT_24H
  const SOL = (Number(amount) / 1e9).toFixed(4)
  ...
  console.log(`  amount:     ${SOL} SOL (${amount} lamports)`)
  console.log(`  last_dep:   ${lastDate} UTC (age: ${ageStr})`)
  console.log(`  count: ${depositCount}, bump: ${bump}`)
